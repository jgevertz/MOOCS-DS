%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Multi-objective synergy optimization by Jana Gevertz (1/25/2023)      %
...
%   "Dose range" for monotherapy is such that each drug loops over PI5  %
%   to PI85.                                                            %
...
%   Figure 4: heatmap of cumulative dose (relative to EC50)             %
%   Figure 5: heatmap of synergy of potency defined by LSD              %
%   Figure 6: heatmap of synergy of potency defined by Loewe            %
%   Figure 7: Contour plot of isobolograms (curves of constant          %
%             combination efficacy)                                     %
%   Figure 8: Pareto front LSD-HSA multi-synergy criterion space        %
%   Figure 9: Doses on LSD-HSA Pareto front                             %
%   Figure 10: Pareto front LSD-Bliss multi-synergy criterion space     %
%   Figure 11: Doses on LSD-Bliss Pareto front                          %
%   Figure 12: Pareto front Loewe-HSA multi-synergy criterion space     %
%   Figure 13: Doses on Loewe-HSA Pareto front                          %
%   Figure 14: Pareto front Loewe-Bliss multi-synergy criterion space   %
%   Figure 15: Doses on Loewe-Bliss Pareto front                        %
%   Figure 16: Number of multi-objective synergy spaces for which each  %
%              dose is Pareto optimal                                   %
...
% - Run time (num_pts = 50): Elapsed time is 1145.995614 seconds.       %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clearvars; clc; close all; tic;
path = 'Output_ToyModel';
if exist(path, 'dir') ~= 7
    mkdir(path)
end
...
[EC50_d1, EC50_d1_TGI] = closest_drug(d1_EC50,1,0.5,r,M,y0,tf,sol_control);
d2_EC50 = linspace(0.05,0.06,10*num_pts); % manualy determined EC50 in this range
[EC50_d2, EC50_d2_TGI] = closest_drug(d2_EC50,2,0.5,r,M,y0,tf,sol_control);
...
[pareto_HSALSD_LSD_plot,pareto_HSALSD_HSA_plot,onPareto_LSD_HSA,onPareto] = ...
    ...
    %find_Pareto(CI_LSD_vec,CI_HSA_vec,CI_LSD,CI_HSA,drug1,drug2,onPareto);
...
[pareto_BlissLSD_LSD_plot,pareto_BlissLSD_Bliss_plot,onPareto_LSD_Bliss,onPareto] = ...
    ...
    %find_Pareto(CI_LSD_vec,CI_Bliss_vec,CI_LSD,CI_Bliss,drug1,drug2,onPareto);
figure; % plot objective function value at each drug dose tested
...
save(fsave,'TGI_drug1','TGI_drug2','TGI_combo','CI_HSA','CI_Bliss','closest_D1',...
    ...
    'pareto_BlissLSD_Bliss_plot','pareto_LoewesHSA_Loewes_plot','pareto_LoewesHSA_HSA_plot',...
    'pareto_LoewesBliss_Loewes_plot','pareto_LoewesBliss_Bliss_plot',...
    'onPareto','onPareto_LSD_HSA','onPareto_LSD_Bliss','onPareto_Loewes_HSA',...
    'onPareto_Loewes_Bliss','CI_LSD_relEC50','min_D_relEC50');
...
function [closest_D, closest_D_TGI] = closest_drug(d,d_num,TGI_target,r,M,y0,tf,sol_control)
