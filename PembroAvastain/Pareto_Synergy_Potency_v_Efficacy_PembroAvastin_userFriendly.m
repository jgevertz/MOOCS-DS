...
%   Figure 6: heatmap of cumulative dose (not useful)                   %
%   Figure 7: heatmap of cumulative relative dose (relative to EC50)    %
%   Figure 8: heatmap of synergy of potency defined by LSD (not useful) %
%   Figure 9: heatmap of synergy of potency defined by relative LSD     %
%   Figure 10: heatmap of synergy of potency defined by Loewe           %
%   Figure 11: Contour plot of isobolograms (curves of constant         %
%              combination efficacy)                                    %
%   Figure 12: Pareto front rel. LSD-HSA multi-synergy criterion        %
%   Figure 13: Doses that fall on relative LSD-HSA Pareto front,        %
%              displayed on TGI heatmap                                 %
%   Figure 14: Pareto front rel. LSD-Bliss multi-synergy criterion space%
%   Figure 15: Doses that fall on relative LSD-Bliss Pareto front       %
%              displayed on TGI heatmap                                 %
%   Figure 16: Pareto front Loewe-HSA multi-synergy criterion space     %
%   Figure 17: Doses that fall on Loewe-HSA Pareto front                %
%              displayed on TGI heatmap                                 %
%   Figure 18: Pareto front Loewe-Bliss multi-synergy criterion space   %
%   Figure 19: Doses that fall on Loewe-Bliss Pareto front              %
%              displayed on TGI heatmap                                 %
%   Figure 20: (Added 10/5) On one plot, shows all doses that appear on %
...
%% DE parameters made globally available so can use in functions
global tmax tStart1 tStart2 p in_cond EC50_d1 EC50_d2 d1_spacing d2_spacing

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Declare model parameters, ICs and dosing protocol                   %%
%% **User can re-define this function to correspond to any model**     %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
set_parameters_ICs_protocol(); 
...
if(p.cell_line == 1) % H1299
    %min_d1 = 0.444; max_d1 = 8.623; % Experiments use 10 mg/kg of pembro (up to EC60)
    min_d1 = 0.444; max_d1 = 10.39; % Experiments use 10 mg/kg of pembro (up to EC66)
    d1 = linspace(min_d1,max_d1,num_pts); % EC5 to EC6x for drug 1 monotherapy
    %min_d2 = 0.1378; max_d2 = 2.576; % Experiments use 1 mpk bevacizumab (up to EC60) 
    min_d2 = 0.1378; max_d2 = 3.071; % Experiments use 1 mpk bevacizumab (up to EC66) 
    d2 = linspace(min_d2,max_d2,num_pts); % EC5 to EC6x for drug 2 monotherapy
%     %% From EC60 range: E(max_d1,max_d2)=0.9391, which is achieved at monotherapy D1=65, D2=12
%     % Dose range for drug 1 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)
%     d1_search = linspace(min(d1),65,30*num_pts); 
%     % Dose range for drug 2 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)  
%     d2_search = linspace(min(d2),12,30*num_pts); 
    %% From EC66 range: E(max_d1,max_d2)=0.9460, which is achieved at monotherapy D1=100.5, D2=13.2
    % Dose range for drug 1 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)
    d1_search = linspace(min(d1),100.5,30*num_pts); 
    % Dose range for drug 2 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)  
    d2_search = linspace(min(d2),13.2,30*num_pts); 
else % A549
    min_d1 = 3.565; max_d1 = 70; % Experiments use 10 mg/kg of pembro
    d1 = linspace(min_d1,max_d1,num_pts); % EC5 to EC60 for drug 1 monotherapy
    min_d2 = 0.19; max_d2 = 3.5; % Experiments use 1 mpk bevacizumab 
    d2 = linspace(min_d2,max_d2,num_pts); % EC5 to EC60 for drug 2 monotherapy
    %% E(max_d1,max_d2)=0.9496, which is achieved at monotherapy D1=1000, D2=20
    % Dose range for drug 1 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)
    d1_search = linspace(min(d1),1000,30*num_pts); 
    % Dose range for drug 2 that ensures we can get monotherapy efficacy = E(max_d1,max_d2)  
...
sol_control  = combinations_TGI(p,in_cond,tmax);
...
for i = 1:length(d1)
    ...
    sol_drug1  = combinations_TGI(p,in_cond,tmax); % standalone drug1
    for j = 1:length(d2)
        p.dose1 = 0;  p.dose2 = d2(j);
        sol_drug2 = combinations_TGI(p,in_cond,tmax); % standalone drug2
        p.dose1 = d1(i);
        sol_combo = combinations_TGI(p,in_cond,tmax);
...
function [] = set_parameters_ICs_protocol()
    global tmax tStart1 tStart2 p in_cond d1_spacing d2_spacing
    tmax = 35; 
    % drug 1 - pembro, 10mpk
...
function [closest_D, closest_D_TGI] = closest_drug(d,d_num,TGI_target)
    global p tmax sol_control in_cond
    dist_closest_D = 10^10; closest_D = -1; closest_D_TGI = -1; 
    for k = 1:length(d)
        if d_num==1            
            p.dose1 = d(k);  p.dose2 = 0;
            sol_d = combinations_TGI(p,in_cond,tmax); % standalone drug1
...
function tumor = combinations_TGI(p,y0,tspan)
    global tStart1 tStart2
    function dydt = ode(t,y0)
        ...
        dD1_t=p.k12_1*D1_p-p.k21_1*D1_t*p.V2_1/p.V1_1;
        ...
        dx1 = p.lamba1*x1*(1-x1/p.K) - x1*kill_term;...%p.lamda0*x1/((1.0+(p.lamda0/p.lamda1*wt)^p.psi)^(1.0/p.psi))... %typical simeoni, what we usually use
