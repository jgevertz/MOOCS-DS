%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Multi-objective synergy optimization (MOOCS-DS)                       %
% Authors: Jana Gevertz and Irina Kareva. Last update: 3/6/2023         %
% - The code first establishes the pembro+beva model, its parameters,   %
%   initial conditions, and dosing protocol. User can adjust these      %
...
%              displayed on TGI heatmap                                 %
%   Figure 18: On one plot, shows all doses that appear on all Pareto   %
%              fronts, with the number shown indicating how many Pareto %
%              fronts the dose fell on                                  %
...
%    appropriately modify the set_parameters_IC_protocol function to    %
%    define the model parameters, initial conditions, and dosing        %
%    protocol. User would also need to determine the range of doses to 	%
%    consider for thier model. Finally, the user needs to ensure their  %
%    code that solves their system of DEs (which should be defined in   %
%    the combination_TGI function) just returns the tumor size at the   %
%    final time point. If all these elements are in place, all the      %
%    combination indices will automatically be computed, and the plots  %
%    will all be made.                                                  %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
num_pts = 30; % 30 number of discrete doses to consider over domain of each dose
[p,in_cond,tmax,tStart1,tStart2,d1_spacing,d2_spacing,d1_experiment,...
    d2_experiment,d1,d2,d1_search,d2_search,min_d1,max_d1,min_d2,max_d2] = ...
...
function [p,in_cond,tmax,tStart1,tStart2,d1_spacing,d2_spacing,d1_experiment,...
    d2_experiment,d1,d2,d1_search,d2_search,min_d1,max_d1,min_d2,max_d2] = ...
    set_parameters_ICs_protocol(num_pts)
    ...
    %% **User must define experimental protocol (dose and spacings) and 
    %% range of doses to consider. We set the smallest dose (min_d1, min_d2)
    %% to be the PI5 dose (5% inhibition relative to control). We set the
    %% largest monotherapy dose (max_d1, max_d2) to be the PI70 dose. For
    %% this model, the result is that the max efficacy of the combination 
    %% therapy is 95%. 
    % Protocol
    ...
    p.dose1 = d1_experiment; % mg/kg; pembro
