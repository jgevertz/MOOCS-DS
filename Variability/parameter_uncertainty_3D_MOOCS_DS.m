%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Variability in MOOCS-DS: This code discetizes parameter space into    %
% voxels, using the parameter bounds established in set_param_ranges.m. %
% It then implements an updated version of the MOOCS-DS multi-objective %
% synergy optimization algorithm.                                       %
% - This version of the code varies all 3 drug-kill parameters: k2_d1   %
%   (pembro monotherapy), k2_d2 (beva monotherapy), and k_3             %
%   (combination). All other parameters are fixed.                      %
% - Parameter space is divided into Nregions-1 = 4 subregions. The      %
%   subregions can be thought of as corresponding to a minimal, mild,   %
%   moderate, and strong response to drug.                              %
% - Nsample parameter combinations are tested in each voxel. They are   %
%   selected using Sobol sampling.                                      %
% - MOOCS-DS is implemented on a num_pts x num_pts discretized region   %
%   of dosing space. Changes from prior implementation of MOOCS-DS:     %
%   1) TGI constraint (removing any combinations for which TGI is below %
%      TGI_thresh) is applied before the multi-objective optimization   %
%      is performed.                                                    %
%   2) Near-Pareto optimal points are also identified.                  %
% - The data gets saved in output.mat in Output_H1299_3D and then it    %
%   needs to be read into analyze_data_3D.m to process the data.        %
%                                                                       %
% Algorithm hyperparameters used in manuscript:                         %
%   Nsamples = 30;                                                      %
%   TGI_thresh = 0.6;                                                   %
%   num_pts = 10;                                                       %
% Run time on a high performance cluster: @12 hours                     %
%                                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load Output_H1299_3D/param_ranges_H1299.mat

% Parameter bounds for growth rate lambda (not used)
params_min(1) = 0.1;     
params_max(1) = 0.4;     

% Parameter bounds for drug 1 kill term, k2_d1 (Nominal: p.k2_d1 = 0.15)
params_min(2) = param_ranges(1,1); 
params_max(2) = param_ranges(1,2);

% Parameter bounds for drug 2 kill term,k2_d2 (Nominal: p.k2_d2 = 0.09)
params_min(3) = param_ranges(2,1);  
params_max(3) = param_ranges(2,2);

% Parameter bounds for combination kill term (Nominal: p.k3 = 0.002)
params_min(4) = param_ranges(3,1); 
params_max(4) = param_ranges(3,2); 

% this discretizes space into (Nregion-1)^Nparams pixels
Nregions = 5; % divides each response into 4 categories:
              % minimal, mild, moderate, strong
param_vec(1,:) = linspace(params_min(2),params_max(2),Nregions);
param_vec(2,:) = linspace(params_min(3),params_max(3),Nregions);
param_vec(3,:) = linspace(params_min(4),params_max(4),Nregions);
Nparams = size(param_vec,1);
Npixels = (Nregions-1)^Nparams;

Ncriterion = 4; % number of multi-objective optimization spaces: do NOT change
Nsamples = 30; % Paper uses 30, but set to 2 for a quicker test
NVPs = Npixels*Nsamples; % number of parameterizations
n_skip = 1000; n_leap = 0; % Parameters needed by sobolset
TGI_thresh = 0.6; 

%% Generate parameterizations
params_all_cell = cell(1, Npixels);
params_pixel_cell = cell(1, Npixels);
count = 0;
for i = 1:Nregions-1
    lb(1) = param_vec(1,i);
    ub(1) = param_vec(1,i+1);

    for j = 1:Nregions-1
        lb(2) = param_vec(2,j);
        ub(2) = param_vec(2,j+1);
    
        for k = 1:Nregions-1
            lb(3) = param_vec(3,k);
            ub(3) = param_vec(3,k+1);
    
            % Generate Nparams parameterizations within voxel
            count = count + 1;
            n_skip = n_skip+(count*Nsamples);
            uniform_sobol = sobolset(Nparams,'Skip',n_skip,'Leap',n_leap);
            uniform_sobol = net(uniform_sobol,Nsamples); % in [0,1]
            % Rescale parameters to be in range [lb,ub]
            uniform_sobol_scaled = zeros(Nsamples,Nparams);
            for l = 1:Nparams
                uniform_sobol_scaled(:,l) = (ub(l) - lb(l))*...
                    uniform_sobol(:,l)+lb(l);
            end
            fprintf('%d. Parameter voxel:\n',count);
            fprintf('\tk2_d1: [%f, %f]\n',lb(1),ub(1));
            fprintf('\tk2_d2: [%f, %f]\n',lb(2),ub(2));
            fprintf('\tk3: [%f, %f]\n',lb(3),ub(3));
            params_all_cell{count} = uniform_sobol_scaled;
            params_pixel_cell{count} = [i j k];
        end
    end
end

%% Process data for VP analysis
params_all = zeros(NVPs,Nparams);  
params_pixel = zeros(NVPs,Nparams);
count = 0;
for i = 1:length(params_all_cell)
    for j = 1:size(params_all_cell{i},1)
        count = count + 1;
        params_all(count,:) = params_all_cell{i}(j,:); % parameter value
        params_pixel(count,:) = params_pixel_cell{i}; % which pixel it is in
    end
end
...
display_graphs = 0; % 1 will show all plots for all VPs
num_pts = 10; % Paper uses 10, but 6 is a decent test run
...
%% Now that path is set, visualize and save parameter plot 
figure; 
set(gcf, 'Units', 'Normalized','OuterPosition', [0.05, 0.05, 0.3, 0.55]);
hold on;
cmap = jet(Npixels); 
p1 = param_vec(1,:);
p2 = param_vec(2,:);
...
for j = 1:length(p2)
    for k = 1:length(p3)
        line([p1(1), p1(end)], [p2(j), p2(j)], [p3(k), p3(k)], ...
...
for i = 1:length(p1)
    for k = 1:length(p3)
        line([p1(i), p1(i)], [p2(1), p2(end)], [p3(k), p3(k)], ...
            'Color', gridColor, 'LineStyle', gridStyle);
    end
end
...
for i = 1:length(p1)
    for j = 1:length(p2)
        line([p1(i), p1(i)], [p2(j), p2(j)], [p3(1), p3(end)], ...
            'Color', gridColor, 'LineStyle', gridStyle);
    end
end
% Plot sampled parameters
for i = 1:Npixels
    scatter3(params_all_cell{i}(:,1),params_all_cell{i}(:,2),...
        params_all_cell{i}(:,3),[],cmap(i,:),'filled')
end
hold off;
...
title('Randomly Sampled Points in Each Region of Parameter Space',...
    'FontSize',16);
view(40,35)
fname_fig = [path '/sampled_parameters'];
saveas(gcf,[fname_fig,'.fig'])
saveas(gcf,[fname_fig,'.png'])

% In this version, tumor growth lambda is fixed across VPs
vec_front = p.lam1*ones(size(params_all,1),1); 
VPs = [vec_front params_all];

%% Run MOOCS-DS for all VPs 
npools = 4; %Open npools parallel pools
...
parfor vp = 1:NVPs
    p = set_parameters(); 
    fprintf('VP #%d: lambda = %f, k2_d_1 = %f, k2_d_2 = %f, k_3 = %f\n',...
        vp,VPs(vp,1),VPs(vp,2),VPs(vp,3),VPs(vp,4));
    ...
    tumor_control = extract_volume(sol_control,0,dose1,dose2); % 0 = no visual;
                                                               % 1 = visualize
    ...
    for obj=1:Ncriterion
        ...
        % Displays Pareto Optimal points found in the loop, along with 
        % corresponding drug doses
...
%% For each VP, count number of times each dose is on or near Pareto front 
font = 'Arial';
...
for vp = 1:NVPs
    ...
    if display_graphs == 1
        ...
        grid off;
        title(['VP #' num2str(vp) ...
            ': Number of Times Dose Appears on Pareto Front'],FontSize',16);
        ...
        ylabel('Dose bevacizumab (mg/kg)','FontSize',16); % d2
        grid off;
        title(['VP #' num2str(vp) ': Number of Times Dose is NEAR Pareto Optimal'],...
            'FontSize',16);
        subtitle('Color Gives TGI of Combination','FontSize',14);
        for i = 1:length(d1)
            for j = 1:length(d2)
        ...
        plot(d1_experiment,d2_experiment,'*r','linewidth',1,'MarkerSize',10);
        hold off; 
    end
end
...
POS_mean = count_onPareto_all/(NVPs*Ncriterion);
POSnear_mean = count_nearPareto_all/(NVPs*Ncriterion);

figure; 
hHM=heatmap(d1,d2,POS_mean');
hHM.YDisplayData=flip(hHM.YDisplayData);
hHM.XLabel = 'Dose pembrolizumab (mg/kg)';
hHM.YLabel = 'Dose bevacizumab (mg/kg)';
hHM.Title ='Mean Pareto Optimality Score';
hHM.CellLabelFormat = '%.2f';
fname_fig = [path '/mean_POS_Vpop'];
saveas(gcf,[fname_fig,'.fig'])
saveas(gcf,[fname_fig,'.png'])

figure; 
hHM=heatmap(d1,d2,POSnear_mean');
hHM.YDisplayData=flip(hHM.YDisplayData);
hHM.XLabel = 'Dose pembrolizumab (mg/kg)';
hHM.YLabel = 'Dose bevacizumab (mg/kg)';
hHM.Title ='Mean Near Pareto Optimality Score';
hHM.CellLabelFormat = '%.2f';
hHM.XDisplayLabels = compose('%.2f', str2double(hHM.XDisplayLabels));
hHM.YDisplayLabels = compose('%.2f', str2double(hHM.YDisplayLabels));
fname_fig = [path '/mean_nearPOS_Vpop'];
saveas(gcf,[fname_fig,'.fig'])
saveas(gcf,[fname_fig,'.png'])
...
save(fsave,'VPs','TGI_drug1','TGI_drug2','TGI_combo','CI_HSA',...
    ...
    'd1','d2','params_all','params_pixel','Npixels',...
    'params_all_cell','param_vec','Nparams','NVPs','Nregions','Nsamples',...
    ...
    'near_pareto_points_Loewe_HSA','near_pareto_points_Loewe_Bliss',...
    'near_pareto_dose_Loewe_Bliss','near_pareto_dose_Loewe_HSA',...
    'count_onPareto_all','count_nearPareto_all','POS_mean','POSnear_mean',...
    'TGI_thresh');
...
function [p,ICs,d1,d2,d1_search,d2_search,min_d1,max_d1,min_d2,max_d2] = ...
    ...
    % free variables for potential future modifications
...
%% Solve model
function sol = solve_model(VP_params,tDose1,cDose1,tDose2,cDose2,...
    tUnion,ti,p,ICs,options) 
    p.lam1  = VP_params(1);
    p.k2_d1 = VP_params(2);
    p.k2_d2 = VP_params(3);
...
function [pareto_points, pareto_dose] = find_pareto(sorted_points)
   ...
   pareto_points = []; 
   pareto_dose = [];
   for i = 1:size(sorted_points, 1) % Iterate over points in sorted_points 
...
function [] = make_plots_Pareto(obj,points,pareto_points,pareto_dose,...
    ...
    scatter(pareto_points(:,1), pareto_points(:,2), 'filled',...
        'MarkerFaceColor', 'r'); 
    plot(pareto_points(:,1), pareto_points(:,2), '-or', 'LineWidth', 0.5);
    legend ('All points', 'Pareto optimal points', 'Location', 'Best');
    ...
    for m = 1:length(d1)
        for n = 1:length(d2)
            for k=1:size(pareto_dose,1)
                if abs(drug1(m,n) - pareto_dose(k,1)) < 1e-6 && ...
                        abs(drug2(m,n) - pareto_dose(k,2)) < 1e-6
    %             fprintf('\tDose (d1,d2)=(%f,%f) appears on Pareto front\n',...
    %                 drug1(m,n),drug2(m,n));
                plot(drug1(m,n),drug2(m,n),'xk','linewidth',2);
                end
            end
        end
    end
...
function [near_pareto_points, near_pareto_dose] = find_near_pareto(points,...
    ...
    if size(pareto_points,1) > 1 % more than one point on Pareto front
        ...
        x_interp = min(pareto_points(:, 1)):min_diff/10:...
            max(pareto_points(:, 1));
        ...
        % Non-Pareto points (objective values)
        non_pareto_points = points(~pareto_idx, 1:2); 
        % Doses corresponding to non-Pareto points
        non_pareto_doses = points(~pareto_idx, 3:4); 
        ...
        % Loop through each non-Pareto point: check if near the Pareto front
